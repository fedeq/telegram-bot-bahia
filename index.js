...
bot.on("message", (msg) => {
  ...
  switch (msg.text.toLocaleLowerCase()) {
    ...
    case "/covid":
      {
        makeRequest(process.env.HOST_COVID, process.env.PATH_COVID_CASOS)
          .then((data) => {
            data = JSON.parse(data);
            const str = data.map((elem) => {
              return `*${capitalize(elem.key)}*: ${elem.values[0].y}`;
            });
            const nuevos_casos = data
              .filter((d) => d["key"] == "nuevos casos")
              .map((c) => c.values);
    ...
    case "/matebot":
      {
        bot.sendMessage(chatId, `Mi hermano no está`, {
          parse_mode: "Markdown",
        });
      }
      break;
    case "/camas":
      {
        makeRequest(process.env.HOST_COVID, process.env.PATH_COVID_CAMAS)
          .then((data) => {
            data = JSON.parse(data);
            let str = "";
    ...
    case "/confirmados":
      {
        makeRequest(process.env.HOST_COVID, process.env.PATH_COVID_CONFIRMADOS)
          .then((data) => {
            data = JSON.parse(data);
            let masculinos = 0;
            let femeninos = 0;
    ...
    case "/dolar":
      {
        makeRequest(process.env.HOST_DOLAR, process.env.PATH_DOLAR)
          .then((data) => {
            data = JSON.parse(data);
            const arr = data.map(
              (elem) =>
                `*${capitalize(elem.casa.nombre)}* - Compra: ${
                  elem.casa.compra
                } - Venta: ${elem.casa.venta}`
            );
    ...
    case "/clima":
      {
        makeRequest(process.env.HOST_CLIMA, process.env.PATH_CLIMA)
          .then((data) => {
            data = JSON.parse(data);
            let msg = "";
